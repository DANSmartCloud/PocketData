...
var Overlays = class {
  ...
  constructor(wotInstance, facadeGetter, domBindings, wtSettings, eventManager, wtTable) {
    ...
    _defineProperty41(this, "resizeObserver", new ResizeObserver((entries) => {
...
function findMatchingTdClose(html2, openEndIndex) {
  let depth = 1;
  let searchStart = openEndIndex;
  while (depth > 0) {
    const tail = html2.substring(searchStart);
    ...
    const openIndex = openMatch ? searchStart + openMatch.index : html2.length;
...
function replaceTdCellsWithTextContent(html2) {
  const result = [];
  let pos = 0;
  while (pos < html2.length) {
    const openMatch = html2.substring(pos).match(TD_OPEN);
    if (!openMatch) {
      result.push(html2.substring(pos));
      break;
    }
    const openStart = pos + openMatch.index;
    const openTag = openMatch[0];
    const openEnd = openStart + openTag.length;
    result.push(html2.substring(pos, openStart));
    const closeInfo = findMatchingTdClose(html2, openEnd);
    if (!closeInfo) {
      result.push(html2.substring(openStart));
      break;
    }
    const cellFragment = html2.substring(openStart, closeInfo.start + closeInfo.length);
...
var SelectionRange = class _SelectionRange {
   ...
   */
  clone() {
    const clone2 = new _SelectionRange(this.createCellRange);
    clone2.ranges = this.ranges.map((cellRange) => cellRange.clone());
    return clone2;
  }
  /**
...
//# sourceMappingURL=chunk-3ZKJIBWV.js.map
