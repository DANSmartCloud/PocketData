...
import { isKeyValueObject } from "../../helpers/object.mjs";
...
import { isUndefined, isDefined, stringify } from "../../helpers/mixed.mjs";
...
export class AutocompleteEditor extends HandsontableEditor {
  ...
  finishEditing(restoreOriginalValue, ctrlDown, callback) {
    if (this.isOpened()) {
      ...
      if (isUndefined(lastSelectedRange) || isDefined(lastSelectedRange) && !lastSelectedRange.includes(this.hot._createCellCoords(this.row, this.col))) {
  ...
  updateChoicesList(choicesList) {
    ...
    for (let i = 0; i < choices.length; i++) {
      const currentItem = isKeyValueObject(choices[i]) ? stripTags(stringify(choices[i].value)) : stripTags(stringify(choices[i]));
