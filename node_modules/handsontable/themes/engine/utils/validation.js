...
exports.validateColorScheme = validateColorScheme;
exports.validateDensityType = validateDensityType;
 ...
 * @type {Set<string>}
 */
const VALID_ICON_KEYS = exports.VALID_ICON_KEYS = new Set(['arrowRight', 'arrowRightWithBar', 'arrowLeft', 'arrowLeftWithBar', 'arrowDown', 'menu', 'selectArrow', 'arrowNarrowUp', 'arrowNarrowDown', 'check', 'checkbox', 'caretHiddenLeft', 'caretHiddenRight', 'caretHiddenUp', 'caretHiddenDown', 'collapseOff', 'collapseOn', 'radio', 'chipClose', 'search']);
...
function validateName(name, context) {
  if (typeof name !== 'string' || name.trim() === '') {
    (0, _errors.throwWithCause)(`[ThemeBuilder] ${context}.name must be a non-empty string.`);
  }
  return name;
}
...
function validateColorScheme(mode) {
  if (!VALID_COLOR_SCHEMES.has(mode)) {
    ...
    (0, _errors.throwWithCause)(`[ThemeBuilder] Invalid color scheme: "${mode}". Must be one of: ${validModes}.`);
  }
  return mode;
}
...
function validateDensityType(type) {
  if (!VALID_DENSITY_TYPES.has(type)) {
    ...
    (0, _errors.throwWithCause)(`[ThemeBuilder] Invalid density: "${type}". Must be one of: ${validTypes}.`);
  }
  return type;
}
...
function validateDensitySizes(sizes, context) {
  if (!(0, _object.isObject)(sizes)) {
    (0, _errors.throwWithCause)(`[ThemeBuilder] ${context}.sizes must be an object.`);
  }
  Object.keys(sizes).forEach(key => {
    if (!VALID_DENSITY_TYPES.has(key)) {
      (0, _console.warn)(`[ThemeBuilder] Unknown density size key: "${key}" in ${context}.sizes. ` + 'This may be a custom density size or a typo.');
    }
  });
}
...
function validateDensityStructure(density, context) {
  if ((0, _object.isObject)(density)) {
    if (!density.type || !density.sizes) {
      (0, _errors.throwWithCause)(`[ThemeBuilder] ${context} must be a string or an object with a 'type' and 'sizes' property.`);
    }
...
function validateColorsStructure(colors, context) {
  Object.entries(colors).forEach(_ref => {
    ...
    if ((0, _object.isObject)(value)) {
      ...
      (0, _errors.throwWithCause)(`[ThemeBuilder] ${currentPath} must be a string or an object.`);
    }
  });
}
...
function validateKeys(obj, validKeys, context) {
  ...
  if (!obj) {
    if (warnMissing) {
      (0, _console.warn)(`[ThemeBuilder] No ${type}s provided in ${context}. All ${type}s are missing.`);
    }
    return;
  }
  ...
  providedKeys.forEach(key => {
    if (!validKeys.has(key)) {
      (0, _console.warn)(`[ThemeBuilder] Unknown ${type} key: "${key}" in ${context}. ` + `This may be a custom ${type} or a typo.`);
    }
  });
  ...
  if (warnMissing) {
    ...
    if (missingKeys.length > 0) {
      (0, _console.warn)(`[ThemeBuilder] Missing ${type}(s) in ${context}: ${missingKeys.join(', ')}`);
    }
  }
}
...
function validateParams(parameters, context) {
  ...
  if (typeof parameters !== 'object' || parameters === null) {
    (0, _errors.throwWithCause)(`[ThemeBuilder] ${context} must be an object.`);
  }
  const {
    name,
    sizing,
  ...
  requiredFields.forEach(field => {
    if (parameters[field] === undefined) {
      (0, _errors.throwWithCause)(`[ThemeBuilder] ${context}.${field} is required.`);
    }
  });
  ...
  // Validate sizing
  if (sizing !== undefined) {
    if (!(0, _object.isObject)(sizing)) {
      (0, _errors.throwWithCause)(`[ThemeBuilder] ${context}.sizing must be an object.`);
    }
  }

  // Validate density
  if (density !== undefined) {
    if (!(0, _object.isObject)(density) && typeof density !== 'string') {
      (0, _errors.throwWithCause)(`[ThemeBuilder] ${context}.density must be a string or an object.`);
    }
  ...
  // Validate icons
  if (icons !== undefined) {
    if (!(0, _object.isObject)(icons)) {
      (0, _errors.throwWithCause)(`[ThemeBuilder] ${context}.icons must be an object.`);
    }
  ...
  // Validate colors
  if (colors !== undefined) {
    if (!(0, _object.isObject)(colors)) {
      (0, _errors.throwWithCause)(`[ThemeBuilder] ${context}.colors must be an object.`);
    }
  ...
  // Validate tokens
  if (tokens !== undefined) {
    if (!(0, _object.isObject)(tokens)) {
      (0, _errors.throwWithCause)(`[ThemeBuilder] ${context}.tokens must be an object.`);
    }
